...
for dir in os.listdir("X:/library/document/卒業制作/"
                      "first_prototype04/train"):
    if dir == ".DS_Store":
        continue
...
# モデルを生成してニューラルネットを構築
model = Sequential()
model.add(Dense(200, input_dim=625))
...
# 学習を実行。10%はテストに使用。
model.fit(image_list, Y, epochs=epoch_size, batch_size=100, validation_split=0.1)
